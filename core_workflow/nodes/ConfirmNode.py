...
async def confirm_node(state: IMState) -> IMState:
    ...
    支持两种模式:
    - **API 模式**: 通过 ConfirmationService 异步等待，由 FastAPI 接口提交确认
    ...
    if confirm_type == "ppt_content" and ppt_content:
        formatted = format_ppt_content(ppt_content)
        display_data = {"type": "ppt_content", "formatted": formatted, "data": ppt_content}
        item_name = "PPT内容"

    # PPT大纲确认
    elif confirm_type == "ppt_outline" and ppt_outline:
        formatted = format_ppt_outline(ppt_outline)
        display_data = {"type": "ppt_outline", "formatted": formatted, "data": ppt_outline}
        item_name = "PPT大纲"
    ...
    try:
        ...
        if confirmation_service.api_mode:
            # ---------- API 模式：注册等待，由接口唤醒 ----------
            req = ConfirmationRequest(
                workflow_id=state["workflow_id"],
                confirm_type=confirm_type,
                display_data=display_data,
                formatted_text=display_data.get("formatted", ""),
            )
            confirmation_service.set_pending(state["workflow_id"], req)
            state["messages"].append(f"[confirm_node] 等待用户确认{item_name} (API模式)")

            # 广播 confirm_required 到前端
            await ws_manager.broadcast_confirm_required(
                workflow_id=state["workflow_id"],
                confirm_type=confirm_type,
            ...
            if source == "feishu_bot" and chat_id:
                await feishu_message_service.send_confirmation_card(
                    chat_id=chat_id,
                    workflow_id=state["workflow_id"],
                    confirm_type=confirm_type,
                    display_data=display_data,
            ...
            if result.get("confirmed"):
                ...
                state["cancelled"] = not bool(result.get("feedback"))
                state["need_confirm"] = False
